...
var SETTLEMENT_TOPIC = keccak256(toBytes("SettlementRequested(uint256,string)"));
function hexToBase64Payload(hex) {
  return Buffer.from(hex.slice(2), "hex").toString("base64");
}
var onHttpTrigger = (runtime2, payload) => {
  runtime2.log("[CreateMarket] HTTP trigger received");
  ...
  const deadline = BigInt(Math.floor(runtime2.now().getTime() / 1000) + durationHours * 3600);
  runtime2.log(`[CreateMarket] "${question}" — deadline: ${deadline}`);
  const callData = encodeAbiParameters(parseAbiParameters("uint8 instr, string question, uint256 deadline, address[] participants"), [0, question, deadline, participants]);
  ...
  const writeResult = evmClient.writeReport(runtime2, {
    receiver: runtime2.config.marketAddress,
    report: signedReport,
    gasConfig: { gasLimit: runtime2.config.gasLimit }
  }).result();
  runtime2.log(`[CreateMarket] ✅ TX status: ${writeResult.txStatus}`);
  return `Market created: "${question}"`;
};
var onLogTrigger = (runtime2, log) => {
  runtime2.log(`[Settle] Log trigger — tx: ${bytesToHex(log.txHash)}`);
  if (log.topics.length < 2) {
    throw new Error("[Settle] Missing indexed topic (marketId)");
  }
  const marketId = BigInt(bytesToHex(log.topics[1]));
  const [question] = decodeAbiParameters(parseAbiParameters("string question"), bytesToHex(log.data));
  runtime2.log(`[Settle] Market ${marketId}: "${question}"`);
  const geminiKey = runtime2.getSecret({ id: "GEMINI_API_KEY" }).result().value;
  const httpCapability = new ClientCapability2;
  const geminiResult = httpCapability.sendRequest(runtime2, (sendRequester, cfg) => buildGeminiRequest(sendRequester, cfg), consensusIdenticalAggregation())({ apiKey: geminiKey, question }).result();
  runtime2.log(`[Settle] Gemini: ${geminiResult.outcome} (${geminiResult.confidence}% confidence)`);
  runtime2.log(`[Settle] Reasoning: ${geminiResult.reasoning}`);
  const outcomeValue = geminiResult.outcome === "Yes" ? 0 : 1;
  const callData = encodeAbiParameters(parseAbiParameters("uint8 instr, uint256 marketId, uint8 outcome"), [1, marketId, outcomeValue]);
  const network248 = getNetwork({
    chainFamily: "evm",
    chainSelectorName: runtime2.config.chainSelectorName,
    isTestnet: true
  ...
  runtime2.log(`[Settle] ✅ ${writeResult.txStatus} — Market ${marketId} → ${geminiResult.outcome}`);
...
function initWorkflow(config) {
  ...
  const httpTrigger = new HTTPCapability().trigger({
    authorizedKeys: config.publicKey ? [{ type: "KEY_TYPE_ECDSA_EVM", publicKey: config.publicKey }] : []
  });
